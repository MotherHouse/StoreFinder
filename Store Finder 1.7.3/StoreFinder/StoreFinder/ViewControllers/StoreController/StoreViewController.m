...
-(NSDictionary*) performParsing {
    if(WILL_DOWNLOAD_DATA && [MGUtilities hasInternetConnection]) {
        ...
        @try {
            ...
            if(loc != nil) {
                NSString* strUrl = [NSString stringWithFormat:@"%@?api_key=%@&lat=%f&lon=%f&radius=%f&category_id=%@",
                                    GET_STORES_JSON_URL,
                                    API_KEY,
...
-(void) setData:(NSDictionary*)dict {
    if(dict != nil) {
        ...
        NSArray* arrayStores = [CoreDataController getStoreByCategoryId:storeCategory.category_id];
