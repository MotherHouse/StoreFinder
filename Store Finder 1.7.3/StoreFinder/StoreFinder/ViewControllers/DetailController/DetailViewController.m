...
- (void)viewDidLoad
{
    ...
    if(store.website != nil && store.website.length > 0)
    ...
    _footerView.buttonCall.enabled = NO;
    if(store.phone_no != nil && store.phone_no.length > 0)
        _footerView.buttonCall.enabled = YES;
    
    _footerView.buttonSMS.enabled = NO;
    if(store.sms_no != nil && store.sms_no.length > 0)
        _footerView.buttonSMS.enabled = YES;
    ...
    _headerView.labelTitle.text = [store.store_name stringByDecodingHTMLEntities];
    _headerView.labelSubtitle.text = [store.store_address stringByDecodingHTMLEntities];
...
-(UITableViewCell*)MGListView:(MGListView *)listView1 didCreateCell:(MGListCell *)cell indexPath:(NSIndexPath *)indexPath {
    ...
    if(cell != nil) {
        ...
        cell.labelDescription.lineBreakMode = NSLineBreakByClipping;
        
        NSString* desc = [store.store_desc stringByDecodingHTMLEntities];
        desc = [desc stringByReplacingOccurrencesOfString:@"\\n" withString:@"\n"];
        CGSize maximumLabelSize = CGSizeMake(self.view.frame.size.width - 20, FLT_MAX);
        CGSize expectedLabelSize = [desc sizeWithFont:[cell.labelDescription font]
                                    constrainedToSize:maximumLabelSize
                                        lineBreakMode:cell.labelDescription.lineBreakMode];
        ...
        if(CLLocationCoordinate2DIsValid(coords)) {
            MGMapAnnotation* ann = [[MGMapAnnotation alloc] initWithCoordinate:coords
                                                                          name:[store.store_name stringByDecodingHTMLEntities]
...
-(CGFloat)MGListView:(MGListView *)listView cell:(MGListCell*)cell heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cellFrame.size.height += expectedLabelSize.height;
...
-(void)didClickButtonRoute:(id)sender {
    ...
    mapItem.name = [store.store_name stringByDecodingHTMLEntities];
...
-(void)didClickButtonEmail:(id)sender {
    ...
    if ([MFMailComposeViewController canSendMail]) {
        ...
        mailController.navigationBar.barStyle = UIBarStyleBlackTranslucent;
        if(DOES_SUPPORT_IOS7) {
            ...
            [[mailController navigationBar ] setTintColor:THEME_MAIN_COLOR];
        }
...
-(void)didClickButtonSMS:(id)sender {
    ...
    if([MFMessageComposeViewController canSendText]) {
        ...
        if(DOES_SUPPORT_IOS7) {
            ...
            [[controller navigationBar ] setTintColor:THEME_MAIN_COLOR];
        }
