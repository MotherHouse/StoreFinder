...
-(NSDictionary*) performParsing {
    if(WILL_DOWNLOAD_DATA && [MGUtilities hasInternetConnection]) {
        ...
        @try {
            ...
            if(loc != nil) {
                NSString* strUrl = [NSString stringWithFormat:@"%@?api_key=%@&lat=%f&lon=%f&radius=%f",
                                    GET_STORES_JSON_URL,
                                    API_KEY,
...
-(void) setData:(NSDictionary*)dict {
    if(dict != nil) {
        ...
        dictEntry = [dict objectForKey:@"categories"];
        for(NSDictionary* dict in dictEntry) {
            [CoreDataController createInstanceStoreCategory:dict];
        }
    ...
    for(Store* store in _arrayData) {
        ...
        if(CLLocationCoordinate2DIsValid(coords)) {
            MGMapAnnotation* ann = [[MGMapAnnotation alloc] initWithCoordinate:coords
                                                                   ...
                                                                   description:[store.store_address stringByDecodingHTMLEntities]];
...
//    Store* store = ann.object;
    ann.mkPinAnnotationView = mKPinAnnotationView;
    
    [self performSelector:@selector(delayLoadPin:) withObject:ann afterDelay:0.2f];
    
//    [self loadMapPinImage:cat.category_icon annotationView:mKPinAnnotationView];
}

-(void)delayLoadPin:(MGMapAnnotation*)ann {
    Store* store = ann.object;
    StoreCategory* cat = [CoreDataController getCategoryByCategoryId:store.category_id];
    [self loadMapPinImage:cat.category_icon annotationView:ann.mkPinAnnotationView];
}
...
-(void) showInfoView:(Store*)store {
    ...
    [_infoSlidingView.labelTitle setText:[store.store_name stringByDecodingHTMLEntities]];
    [_infoSlidingView.labelSubtitle setText:[store.store_address stringByDecodingHTMLEntities]];
