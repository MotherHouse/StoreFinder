...
- (void)viewDidLoad
{
    ...
    int maxDistance = [delegate getFilterDistance];
    
    _searchView.sliderRadius.value = maxDistance / 2;
    _searchView.sliderRadius.maximumValue = maxDistance;
    _searchView.sliderRadius.minimumValue = 0;
...
-(void)doSearch:(NSMutableArray*) arrayFilter {
    ...
    for(Store* store in arrayStores) {
        ...
        BOOL isFoundKeyword = ([[store.store_name stringByDecodingHTMLEntities] containsString:strKeywords] ||
                               [[store.store_address stringByDecodingHTMLEntities] containsString:strKeywords] );
...
-(NSDictionary*) performParsing {
    if(WILL_DOWNLOAD_DATA && [MGUtilities hasInternetConnection]) {
        ...
        @try {
            ...
            if(loc != nil) {
                NSString* strUrl = [NSString stringWithFormat:@"%@?api_key=%@&lat=%f&lon=%f&radius=%f&fetch_category=1",
                                    GET_STORES_NEWS_JSON_URL,
                                    API_KEY,
...
-(void) setData:(NSDictionary*)dict {
    ...
    [_searchView.pickerCategories reloadAllComponents];
